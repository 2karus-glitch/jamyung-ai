...
# 📸 [디자인 업그레이드] 세로형 전통 큰북의 웅장한 고화질 그래픽을 최상단에 배치
...
st.caption("K-Cloud 기반 한국어 맥락 인지 엔진 v1.5 - 마스터 통합판")
st.markdown("---")
...
# 4. 실시간 크롤러 및 링크 바인딩 엔진
def fetch_realtime_data(keyword, s_date, e_date, mode):
    if "가상" in mode:
        backup_data = [
    ...
    # 네이버 뉴스 파싱 및 주소 매칭
    try:
        news_url = f"https://naver.com/search.naver?where=news&query={keyword}&sm=tab_opt&sort=1&ds={s_str}&de={e_str}"
        res = requests.get(news_url, headers=headers, timeout=5)
        if res.status_code == 200:
            soup = BeautifulSoup(res.text, 'html.parser')
            articles = soup.select('.news_wrap')
            for art in articles[:5]:
                title_el = art.select_one('.news_tit')
                title = title_el.text if title_el else ""
                link_url = title_el['href'] if title_el and title_el.has_attr('href') else "https://naver.com"
                dsc = art.select_one('.api_txt_lines.dsc_txt_wrap').text if art.select_one('.api_txt_lines.dsc_txt_wrap') else ""
                if title or dsc:
                    scraped_data.append({'platform': 'Naver News', 'text': f"{title} {dsc}", 'url': link_url})
    except: pass
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            for post in posts[:5]:
                title_el = post.select_one('.tit_txt')
                title = title_el.text if title_el else ""
                link_url = title_el['href'] if title_el and title_el.has_attr('href') else "https://dcinside.com"
                desc = post.select_one('.desc_txt').text if post.select_one('.desc_txt') else ""
                if title or desc:
                    scraped_data.append({'platform': 'Dcinside', 'text': f"{title} {desc}", 'url': link_url})
    except: pass
...
if st.button("🏛️ 자명고 통계 검정 및 스캔 시작", use_container_width=True):
    with st.spinner("실시간 여론 데이터 동기화 및 렌더링 중..."):
        df_real = fetch_realtime_data(target_keyword, start_date, end_date, mode_option)
        time.sleep(1)
    
    if df_real.empty:
        st.warning("선택하신 기간 내 수집된 원문 데이터가 없습니다. 모드를 '가상 시뮬레이션 모드'로 변경해 주세요.")
    else:
        ...
        # HTML 렌더링을 통해 실제 클릭 가능한 표로 프론트엔드 표기
